...
import org.sail.mq.proxy.config.Configuration;
import org.sail.mq.proxy.config.ConfigurationManager;
import org.sail.mq.proxy.config.ProxyConfig;
import org.sail.mq.proxy.grpc.GrpcServer;
import org.sail.mq.proxy.grpc.GrpcServerBuilder;
import org.sail.mq.proxy.grpc.v2.GrpcMessagingApplication;
import org.sail.mq.proxy.metrics.ProxyMetricsManager;
...
public class ProxyStartup {
    ...
    public static void main(String[] args) {
        try {
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                ...
                try {
                    ...
                    log.error("err when shutdown sailmq-proxy", e);
                }
            }));
        } catch (Exception e) {
            e.printStackTrace();
        ...
        System.out.printf("%s%n", new Date() + " sailmq-proxy startup successfully");
        log.info(new Date() + " sailmq-proxy startup successfully");
    }
