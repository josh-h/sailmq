...
public class RouteActivity extends AbstractMessingActivity {
    ...
    public CompletableFuture<QueryRouteResponse> queryRoute(ProxyContext ctx, QueryRouteRequest request) {
        ...
        try {
            ...
            List<org.sail.mq.proxy.common.Address> addressList = this.convertToAddressList(request.getEndpoints());
    ...
    protected List<org.sail.mq.proxy.common.Address> convertToAddressList(Endpoints endpoints) {

        boolean useEndpointPort = ConfigurationManager.getProxyConfig().isUseEndpointPortFromRequest();

        List<org.sail.mq.proxy.common.Address> addressList = new ArrayList<>();
        for (Address address : endpoints.getAddressesList()) {
            int port = ConfigurationManager.getProxyConfig().getGrpcServerPort();
            if (useEndpointPort) {
                port = address.getPort();
            }
            addressList.add(new org.sail.mq.proxy.common.Address(
                org.sail.mq.proxy.common.Address.AddressScheme.valueOf(endpoints.getScheme().name()),
    ...
    protected Map<String /*brokerName*/, Map<Long /*brokerID*/, Broker>> buildBrokerMap(
        ...
        for (ProxyTopicRouteData.ProxyBrokerData brokerData : brokerDataList) {
            ...
            for (Map.Entry<Long, List<org.sail.mq.proxy.common.Address>> entry : brokerData.getBrokerAddrs().entrySet()) {
                Long brokerId = entry.getKey();
                List<Address> addressList = new ArrayList<>();
                AddressScheme addressScheme = AddressScheme.IPv4;
                for (org.sail.mq.proxy.common.Address address : entry.getValue()) {
